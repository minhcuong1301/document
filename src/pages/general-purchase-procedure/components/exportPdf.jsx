    ...
    <View style={styles.table}>
      ...
      {data?.map((item, index) =>
        <View key={item?.id} style={styles.tableRow}>
          ...
          <TabCol width={80}>
            <Text>{item?.estimated_price ? formatCurrency(item?.estimated_price) : ""}</Text>
          </TabCol>
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <View>
        ...
        <Text style={styles.commonText}>Thời đề xuất: {moment(data?.time_created * 1000).format(DATETIME_FORMAT)}</Text>
      </View>
