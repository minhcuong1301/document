...
import {
  actionGetGeneralPurchaseProcedures,
...
const GeneralPurchaseProcedure = () => {
  ...
  const handleGetGeneralPurchaseProcedures = async (page_num, page_size) => {
    setSpinning(true);
    try {
      ...
      const { data, status } = await actionGetGeneralPurchaseProcedures(params);

      if (status === 200) {
        setDataTb2(data?.procedures);
        console.log("dexuat da gui:", data?.procedures);
      ...
      <Col span={24}>
        <Table
          ...
          pagination={{
            ...
            onChange: handleGetGeneralPurchaseProcedures,
          }}
          scroll={{ x: 1024 }}
        />
      </Col>
  ...
  useEffect(() => {
    if (userLogin.position_code !== "GIAM_DOC") {
      handleGetGeneralPurchaseProcedures(1, 10);
    }
    ...
    <Layout className="common-layout">
      <SpinCustom spinning={spinning}>
        ...
        <div className="common-layout--content">
          <Tabs items={TabItem} onTabClick={(e) => handleSelectedTabKey(e)} />
        </div>
      </SpinCustom>
...
export default GeneralPurchaseProcedure;
