    ...
    <View style={styles.table}>
      ...
      {data?.map((item, index) =>
        <View key={item?.id} style={styles.tableRow}>
          ...
          <TabCol width={80}>
            <Text renderText={handleRenderText}>{NOTARIZATION_PROCEDURE_TYPES[item?.type_notariza]}</Text>
          </TabCol>

          <TabCol width={70}>
            <Text>{item?.day_need ? moment(item?.day_need * 1000).format(DATE_FORMAT) : ""}</Text>
          </TabCol>
...
const ExportPdfModal = ({ data, onCancel }) => {
  console.log('data tab cong chung:',data);
